...
class Current:
    ...
    @classmethod
    def get(cls):
        if (SCRIPT_DIR / "colors.json").is_file():
            with (SCRIPT_DIR / "colors.json").open("r") as f:
                return cls(**json.load(f))
        else:
            return cls("green", "#78dba9")

    def set(self, name, pal):
        self.name = name
        self.hex = pal[name]
        with (SCRIPT_DIR / "colors.json").open("w") as f:
            json.dump(self.__dict__, f)


with (SCRIPT_DIR / "config.json").open("rb") as f:
    config = json.load(f)
